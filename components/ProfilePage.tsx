...
const ProfilePage: React.FC<ProfilePageProps> = ({ currentUser, onBack, onProfileUpdate, onLogout }) => {
  ...
  const refreshProfileData = async () => {
    ...
    try {
      ...
      // This prevents unnecessary re-renders and state changes
      if (onProfileUpdate && JSON.stringify(freshProfile) !== JSON.stringify(currentUser)) {
        console.log('🔄 Profile data changed, updating parent component');
        onProfileUpdate(freshProfile as AuthUser);
      } else {
        console.log('✅ Profile data unchanged, no parent update needed');
      }
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-green-600 text-white">
        <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4">
          ...
          <button
            ...
            className="p-2 rounded-full hover:bg-green-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        {/* Refresh Status */}
        {refreshedProfile && (
          <div className="text-xs text-green-200 text-center pb-2">
        ...
        {process.env.NODE_ENV === 'development' && (
          <div className="text-xs text-green-200 text-center pb-2">
            Debug: ProfilePage mounted, showProfilePage should be true
          </div>
        )}
      </div>
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <div className="relative mb-4 sm:mb-6">
            {refreshedProfile?.profile_photo_url ? (
              ...
              <div className="w-20 h-20 sm:w-24 sm:h-24 lg:w-32 lg:h-32 rounded-full overflow-hidden border-4 border-green-600">
                <img 
                ...
                <div className="w-full h-full bg-gradient-to-br from-green-400 to-green-600 rounded-full flex items-center justify-center text-white text-xl sm:text-2xl lg:text-3xl font-bold hidden">
                  {refreshedProfile?.name?.charAt(0)?.toUpperCase() || refreshedProfile?.email?.charAt(0)?.toUpperCase() || 'U'}
                </div>
              </div>
            ) : (
              // Show default avatar
              <div className="w-20 h-20 sm:w-24 sm:h-24 lg:w-32 lg:h-32 bg-gradient-to-br from-green-400 to-green-600 rounded-full flex items-center justify-center text-white text-xl sm:text-2xl lg:text-3xl font-bold">
                {refreshedProfile?.name?.charAt(0)?.toUpperCase() || refreshedProfile?.email?.charAt(0)?.toUpperCase() || 'U'}
              </div>
            )}
            {/* Debug info */}
            {refreshedProfile?.profile_photo_url && (
              <div className="text-xs text-gray-500 mt-2 text-center">
                Debug: Photo URL exists
              </div>
            )}
            <button 
              className="absolute bottom-0 right-0 w-6 h-6 sm:w-8 sm:h-8 lg:w-10 lg:h-10 bg-green-600 rounded-full flex items-center justify-center text-white hover:bg-green-700 transition-colors"
              onClick={() => setShowEditModal(true)}
            >
          ...
          {(refreshedProfile?.ca_code || refreshedProfile?.cs_code) && (
            <div className="flex flex-col sm:flex-row items-center gap-2 mb-4 sm:mb-6">
              <span className="text-xs sm:text-sm text-slate-500">
                {refreshedProfile?.role === 'CA' ? 'CA Code:' : 'CS Code:'}
              </span>
      ...
      {(refreshedProfile?.government_id || refreshedProfile?.ca_license || refreshedProfile?.cs_license || (refreshedProfile?.role === 'CS' && refreshedProfile?.ca_license) || (refreshedProfile?.verification_documents && refreshedProfile.verification_documents.length > 0)) && (
        <div className="bg-white mt-4 px-4 sm:px-6 py-6">
          ...
          <div className="space-y-3">
            ...
            {refreshedProfile?.ca_license && refreshedProfile?.role === 'CA' && (
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 p-3 bg-blue-50 border border-blue-200 rounded-md">
                <Shield className="h-5 w-5 text-blue-600 flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-blue-800">CA License</p>
                  <p className="text-xs text-blue-600 break-all">{refreshedProfile.ca_license}</p>
                </div>
                <button
                  onClick={() => window.open(refreshedProfile.ca_license, '_blank')}
            ...
            {/* CS License - Check both cs_license and ca_license fields for CS users */}
            {refreshedProfile?.role === 'CS' && (refreshedProfile?.cs_license || refreshedProfile?.ca_license) && (
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 p-3 bg-blue-50 border border-blue-200 rounded-md">
                <Shield className="h-5 w-5 text-blue-600 flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-blue-800">CS License</p>
                  <p className="text-xs text-blue-600 break-all">{refreshedProfile.cs_license || refreshedProfile.ca_license}</p>
                </div>
                <button
                  onClick={() => window.open(refreshedProfile.cs_license || refreshedProfile.ca_license, '_blank')}
                  className="text-blue-600 hover:text-blue-800 text-sm underline flex-shrink-0"
                >
                  View
                </button>
